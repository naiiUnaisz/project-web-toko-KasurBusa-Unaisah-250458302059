        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @endif
                    <form wire:submit="createSize" class="mb-6">
                                ...
                                <tr wire:key="{{ $item->id }}"> <td class="px-6 py-4 whitespace-nowrap">{{ $item->id }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($editingSizeId === $item->id)
                                            <input type="text" wire:model="editingSizeName" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm">
                                            @error('editingSizeName')
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if ($editingSizeId === $item->id)
                                            <button wire:click="updateSize" class="text-green-600 hover:text-green-900">Simpan</button>
                                            <button wire:click="cancelEditSize" class="text-gray-600 hover:text-gray-900 ml-2">Batal</button>
                                        @else
                                            <button wire:click="editSize({{ $item->id }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                                            <button wire:click="deleteSize({{ $item->id }})" 
                                                    wire:confirm="Yakin ingin menghapus Ukuran ini?"
